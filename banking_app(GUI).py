...
def fin_sign():
    ...
    if(re.search(regex, email)) is None:
        notifications.config(fg='red', text="Your Email address is not valid. PLEASE enter the valid Email Address!")
        return

    for check_name in accounts:
        newfile = f'{name}.csv'
        if newfile == check_name:
            notifications.config(fg='red', text="Sorry somebody already has this USERNAME!")
            return
            ...
            File.close()
            notifications.config(fg='green', text="Congratulations Your Account is successfully Created")
...
def Register():
    ...
    Label(RegisterScreen, text="Email", font=(30)).grid(row=3, sticky=W, pady=10)
    Label(RegisterScreen, text="Password", font=(30)).grid(row=4, sticky=W, pady=10)

    Entry(RegisterScreen, textvar=TempName).grid(row=2, column=2)
    Entry(RegisterScreen, textvar=TempEmail).grid(row=3, column=2)
    Entry(RegisterScreen, textvar=TempPw, show="*").grid(row=4, column=2)

    Button(RegisterScreen, text="Confirm", command=fin_sign,font=(20)).grid(row=7, sticky=N, pady=10)
    Button(RegisterScreen, text="Show Password", command=showsign,font=(18)).grid(row=6, sticky=W, pady=10)
    Button(RegisterScreen, text="Hide Password", command=hidesign,font=(18)).grid(row=6, column=4, sticky=E, pady=10)
    notifications = Label(RegisterScreen, font=(30))
    notifications.grid(row=8, sticky=N, pady=10)

def showsign():
    Entry(RegisterScreen, textvar=TempPw, show="").grid(row=4, column=4)

def hidesign():
    Entry(RegisterScreen, textvar=TempPw, show="*").grid(row=4, column=4)

def showsignlog():
    Entry(LogInScreen, textvar=TempLogPw, show="").grid(row=2, column=4)
    ...
    for check_name in accounts:
        if check_name == CheckLogName:
            ...
            if LogPw == CorrectPassword:
                ...
                Label(dashboard, text="DASHBOARD:", font=30).grid(row=0, sticky=W, pady=5)
                Label(dashboard, text=f"Welcome {LogName}", font=20).grid(row=0, sticky=N, pady=5)
...
def account_info():
    ...
    Label(info, bg='orange', text=f"Name: {name_info}", font=40).grid(row=1, sticky=W, pady=10, padx=30)
    Label(info, bg='orange', text=f"Password: {email_info}", font=40).grid(row=2, sticky=W, pady=10, padx=30)
    Label(info, bg='orange', text=f"Email: {pw_info}", font=40).grid(row=3, sticky=W, pady=10, padx=30)
...
def deposit():
    ...
    Label(deposit_display_screen, text="Deposit amount: ", font=20).grid(row=0, sticky=N, pady=10)
    current_rands_label = Label(deposit_display_screen, text=f"Your currently balance is now:R {balance_details}", font=(20))
...
def fin_deposit():
    ...
    if float(rands_to_enter.get()) <= 0:
        ...
        return
    with open(f'{LogName}.csv', 'r+') as file:
        data = file.read()
        info = data.split('\n')
        old_balance = info[3]
        ...
        data = data.replace(old_balance, new_balance)
        file.seek(0)
        file.truncate(0)
        file.write(data)
    current_rands_label.config(text=f"Your now current balance is:R{new_balance}", fg="green")
    deposit_notifications.config(text="Deposit was successfully made", fg="green")
...
def withdraw():
    ...
    current_rands_label = Label(withdraw_display_screen, text="Deposit amount: ", font=(20))
    current_rands_label.grid(row=0, sticky=N, pady=10)
    Label(withdraw_display_screen, text="withdraw: ",font=(20)).grid(row=2, sticky=W, pady=10)
    withdraw_rands_label = Label(withdraw_display_screen, text=f"You currently have:R {balance_details}", font=(20))
...
def fin_withdraw():
    ...
    with open(f'{LogName}.csv', 'r+') as file:
        ...
        new_balance = str(round(new_balance, 2))
        if float(new_balance) < 0:withdraw_notifications.config(text="Sorry...you can not WITHDRAW more than you have", fg="red")
        return
    data = data.replace(old_balance, new_balance)
    file.seek(0)
    file.truncate(0)
    file.write(data)
    file.close()
    withdraw_rands_label.config(text=f"You currently have:R {new_balance}", fg="green")
    withdraw_notifications.config(text="The withdrawal was successfully processed", fg="green")
...
def balance():
    ...
    Label(balance, bg='green', text=f"Balance:R {balance_info}", font=(60)).grid(row=3, sticky=W, pady=10, padx=30)
